...
for (i in c(1:dim(info_list_files1)[1])){
	...
	### cell type
	filename[i] = unlist(strsplit(toString(info_list_files1[i,1]), "[/]"))[1]
...
ymin_FSiP = min(c(sig1_fsip, sig2_fsip, sig3_fsip))-0.1
ymax_FSiP = max(c(sig1_fsip, sig2_fsip, sig3_fsip))+0.1
...
FSiP = cbind(sig1_fsip, sig2_fsip, sig3_fsip)
