...
t_r_curve_change_point = function(t_r_matrix, changepoint_method, max_cp_num, t_r_change_point_plot_file_name, ignore_t_lim_lower, ignore_t_lim_upper, raw_plot_lim, mean_or_var, fit_polynorm, polynomial_degree){
	...
	par(mfrow=c(1,1))
	heatscatter(t, r, pch = 20, ylim=c(-raw_plot_lim,raw_plot_lim), log='x', main=paste(toString(t[ansvar[1]]), 'VS', toString(t[ansvar_norm[1]]), sep=' ') )
	...
	par(mfrow=c(1,1))
	heatscatter(t, r-lo_fit_value, pch = 20, ylim=c(-raw_plot_lim,raw_plot_lim), log='x', main=toString(t[ansvar_norm[1]]))
	abline(v = t_variance_change_point, col = 'red', lty=2)
	dev.off()
...
MAnorm = function(data_x_sig, data_y_sig, sampling_num, seed, MAplot_output_file_name){
	...
	heatscatter(log2(data_x_sig[used_id,1] + small_num), log2(data_y_sig[used_id,1] + small_num), pch = 20, main='original signal')
	abline(0,1,col = 'blue')
	dev.off()	
